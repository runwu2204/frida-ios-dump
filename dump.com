...
function getDocumentDir() {
  ...
  return frida_objc_bridge_default.Object(npdirs).objectAtIndex_(0).toString();
}
...
function loadAllDynamicLibrary(app_path) {
  var defaultManager = frida_objc_bridge_default.classes.NSFileManager.defaultManager();
  ...
  for (var i = 0, l = filenames.count(); i < l; i++) {
    ...
    if (file_name.hasSuffix_(".framework")) {
      var bundle = frida_objc_bridge_default.classes.NSBundle.bundleWithPath_(file_path);
      if (bundle.isLoaded()) {
...
function handleMessage(message) {
  modules = getAllAppModules();
  var app_path = frida_objc_bridge_default.classes.NSBundle.mainBundle().bundlePath();
